...
const mediaRows = (clause: string, media: string, lot: string): CheckItem[] => [
  compliant({
    ...
    checkDescription: `QRA read the media record for ${media}. A plate is only as good as what was poured into it, so the lot number is checked against the preparation log rather than taken from the worksheet alone.`,
  ...
  compliant({
    ...
    statusText: "Recorded",
    checkDescription:
      "QRA read the preparation date against the day of use. Media prepared too far ahead of the analysis can dry or lose selectivity before the sample ever reaches it.",
  ...
  compliant({
    ...
    checkDescription:
      "QRA read the expiry against the day of use. Expired media under-recovers, so a clean plate proves nothing about the sample.",
...
const equipmentRows = (
  ...
  const rows: CheckItem[] = [
    compliant({
      ...
      checkDescription: `QRA read the equipment record for ${instrument} against its calibration state. An incubator running warm or an air sampler drawing the wrong volume changes the count without changing anything on the worksheet.`,
      expected: "Equipment identified and within calibration at the date of use",
      actual: `${instrument} — calibrated, due 12-Jan-2027`,
      expectedSource: sopFor(clause),
      source: "Caliber LIMS",
      comparison: "Equipment identity and calibration read against the usage record",
    }),
  ];
  ...
  if (neutraliser) {
    rows.push(
      compliant({
        ...
        statusText: "Recorded",
        checkDescription:
          "QRA read the neutraliser record. Without an effective neutraliser, residual product carried onto the plate suppresses growth and the count reads low for a reason that has nothing to do with the sample.",
        expected: "Neutraliser identified with its lot and efficacy record",
        actual: "Lecithin / polysorbate 80 — lot NEU-2026-0071, efficacy verified",
        expectedSource: sopFor(clause),
        source: "Caliber LIMS",
...
const controlRows = (clause: string, positive: boolean): CheckItem[] => {
  ...
  if (positive) {
    rows.push(
      compliant({
        ...
        checkDescription:
          "QRA read the positive control result. It is what shows the media and the method can recover an organism at all — without it a clean sample plate proves nothing.",
  ...
  rows.push(
    compliant({
      ...
      checkDescription:
        "QRA read the negative control result. Growth here would mean the plate, the diluent or the handling introduced the organism, and every count in the set would be unreadable.",
...
const analysisRecordRows = (clause: string, label = "Analysis"): CheckItem[] => [
  compliant({
    prefix: P,
    sopReference: sopFor(clause),
    ...
    checkDescription:
      "QRA read the start and completion dates against the incubation period the method requires. A plate read early under-counts; one read late can overgrow.",
  ...
  compliant({
    ...
    checkDescription:
      "QRA read the signature record. Two signatures are what makes the result somebody's rather than the system's.",
...
const logBookSection = (
  ...
  section(parameter, "Log Books", order, [
    ...
    compliant({
      ...
      checkDescription: `QRA read the usage log for ${instrument}. An analysis with no usage entry against it cannot be placed on a particular instrument on a particular day.`,
    ...
    compliant({
      ...
      checkDescription:
        "QRA read the media preparation log against what was issued and used. Reconciliation is what catches plates poured and never accounted for.",
      expected: "Prepared, issued and used quantities reconcile",
      actual: "Prepared 40 plates, issued 32, used 32, discarded 8 — reconciled",
      expectedSource: sopFor("8"),
      source: "Caliber LIMS",
      comparison: "Preparation, issue and usage quantities read against each other",
    }),
  ]);
...
const sections: Section[] = [
  ...
  section("mlt", "Sample Preparation", 3, [
    compliant({
      ...
      checkDescription:
        "QRA read the weight print against the worksheet. The count is reported per gram, so an error in the weight moves the result without touching a plate.",
    ...
    compliant({
      ...
      statusText: "Verified",
      checkDescription:
        "QRA read the dilution against the reported count. The dilution factor multiplies straight into the result, so an unverified one carries all the way through.",
  ...
  section("mlt", "MLT Results", 4, [
    compliant({
      ...
      checkDescription:
        "QRA read the TAMC against its limit. A count inside the limit still has to be read with the controls beside it — the plate only means something if the positive control grew.",
    ...
    compliant({
      ...
      statusText: "Within limit",
      checkDescription:
        "QRA read the TYMC against its limit. Mould recovers more slowly than bacteria, which is why it is counted on its own plate and against its own number.",
    ...
    compliant({
      ...
      statusText: "Absent",
      checkDescription:
        "QRA read the pathogen result. This one is not a count but a presence test — a single organism recovered is a finding whatever the total count says.",
  ...
  logBookSection("mlt", 6, "Soybean casein digest agar", "SCDA-2026-0812", "Incubator INC-MB-004"),
  ...
  section("cleaning", "Sample Collection", 3, [
    compliant({
      ...
      statusText: "Swab",
      checkDescription:
        "QRA read which sample type was taken. A swab and a rinse recover differently and are reported against different limits, so the type has to be on the record before the count means anything.",
    ...
    compliant({
      ...
      statusText: "Recorded",
      checkDescription:
        "QRA read the collection time against the start of incubation. A swab left too long before plating loses recoverable organisms.",
      expected: "Collection time and date recorded, plated within the method window",
  ...
  section("cleaning", "Cleaning Validation Results", 4, [
    compliant({
      ...
      statusText: "Within limit",
      checkDescription:
        "QRA read the TMC against the surface limit. Cleaning validation is asking whether the previous product and its organisms are gone, so the number is per unit area rather than per gram.",
    ...
    compliant({
      ...
      statusText: "Absent",
      checkDescription:
        "QRA read the pathogen result for the cleaned surface. Equipment that carries a pathogen forward contaminates every batch that follows, not only this one.",
  ...
  logBookSection("cleaning", 6, "Soybean casein digest agar", "SCDA-2026-0812", "Incubator INC-MB-004"),
  ...
  section("watermicro", "Media and Equipment", 1, [
    ...
    compliant({
      ...
      statusText: "Recorded",
      checkDescription:
        "QRA read the filtered volume. The count is reported per 100 mL, so a different volume through the filter changes the result arithmetically.",
    ...
    compliant({
      ...
      statusText: "Recorded",
      checkDescription:
        "QRA read the filter specification. A pore size above 0.45 µm lets organisms through and the plate under-counts without any sign that it has.",
  ...
  section("watermicro", "Water Microbiology Results", 3, [
    compliant({
      ...
      statusText: "Within limit",
      checkDescription:
        "QRA read the TMC against the limit for this water grade. Purified water is not sterile; the question is whether the count is where the system normally holds it.",
    ...
    compliant({
      ...
      statusText: "Absent",
      checkDescription:
        "QRA read the pathogen result for the water sample. A pathogen in the water system reaches every product made with it.",
  ...
  section("watermicro", "Sampling Schedule", 4, [
    flagged({
      ...
      checkDescription:
        "QRA looked for confirmation that this monitoring point was sampled on its scheduled rotation. A point sampled off-schedule produces a count that cannot be trended against the ones before it.",
      ...
      expectedSource: sopFor("3"),
      comparison:
        "QRA found no schedule confirmation against the monitoring point for this sampling round",
  ...
  logBookSection("watermicro", 6, "R2A agar", "R2A-2026-0804", "Filtration manifold FM-MB-002"),
  ...
  section("em", "EM Results", 3, [
    flagged({
      ...
      reference: "28 CFU/m³",
      checkDescription:
        "QRA read the total bacterial count against the alert limit for this grade of area. A count over the limit says the room was not holding its condition while the product was exposed to it.",
      ...
      table: {
        ...
        rows: [
          {
            cells: ["Total Bacterial Count", "28 CFU/m³", "NMT 25 CFU/m³", "OOL"],
            flagged: true,
          },
          { cells: ["TYMC", "2 CFU/m³", "NMT 50 CFU/m³", "Within limit"] },
          { cells: ["Total Viable Count", "30 CFU/m³", "NMT 100 CFU/m³", "Within limit"] },
        ],
      },
    }),
    compliant({
      ...
      statusText: "Within limit",
      checkDescription:
        "QRA read the mould count against its limit. Mould in an air sample points at the fabric of the room rather than at the people in it, which is a different investigation from a bacterial exceedance.",
    ...
    compliant({
      ...
      statusText: "Within limit",
      checkDescription:
        "QRA read the total viable count against its limit. It sits inside its own limit even though the bacterial count above does not, which is why both are reported rather than one standing for the other.",
  ...
  section("em", "Analysis Record", 4, analysisRecordRows("4", "Environmental monitoring")),
  logBookSection("em", 5, "Soybean casein digest agar", "SCDA-2026-0812", "Air sampler AS-MB-001"),
  ...
  section("waterchem", "Description and Physical", 1, [
    compliant({
      ...
      statusText: "Complies",
      checkDescription:
        "QRA read the visual description. Turbidity or colour in purified water is the first sign of a system problem and is seen before any instrument reports it.",
    ...
    compliant({
      ...
      statusText: "Within limit",
      checkDescription:
        "QRA read the pH against its range. Purified water is weakly buffered, so a pH outside the range usually means something has been picked up from the system rather than from the sample.",
    ...
    compliant({
      ...
      statusText: "Within limit",
      checkDescription:
        "QRA read the conductivity against its limit. It is the quickest indicator that ionic material has entered the loop, which is why it is trended rather than simply passed.",
  ...
  section("waterchem", "Nitrates and Nitrites", 2, [
    compliant({
      ...
      statusText: "Within limit",
      checkDescription:
        "QRA read the nitrate result and the standard it was measured against. A colourimetric result is only as good as the standard behind it, so the preparation is checked with the number.",
      expected: "Not more than 0.2 ppm — STP-MB-WC-005",
      actual: "0.12 ppm — standard prepared 20-Aug-2026, lot NIT-2026-0033",
      expectedSource: "STP-MB-WC-005",
      source: "Caliber LIMS",
      comparison: "Result read against the limit, standard preparation verified",
    }),
    compliant({
      prefix: P,
      sopReference: sopFor("5"),
      ...
      statusText: "Within limit",
      checkDescription:
        "QRA read the nitrite result and its standard. Nitrite indicates biological activity in the loop, so a rising figure matters even while it is inside the limit.",
  ...
  section("waterchem", "TOC", 3, [
    compliant({
      ...
      statusText: "Within limit",
      checkDescription:
        "QRA read the TOC against its limit. Organic carbon is the measure of what the loop is growing or leaching, so it is read alongside the microbial count rather than separately from it.",
    ...
    flagged({
      ...
      checkDescription:
        "QRA looked for the analyser print behind the TOC figure. The transcribed number cannot be checked against anything without it, so the result stands on somebody's typing.",
  ...
  section("waterchem", "Additional Parameters", 4, [
    compliant({
      ...
      statusText: "Complies",
      checkDescription:
        "QRA read the acidity/alkalinity result. It catches ionic contamination that a single pH reading can sit inside without showing.",
    ...
    compliant({
      ...
      statusText: "Complies",
      checkDescription:
        "QRA read the heavy metals result. Metal picked up from the distribution loop carries into every product made with the water.",
      expected: "Complies — STP-MB-WC-005",
      actual: "Complies",
      expectedSource: "STP-MB-WC-005",
      source: "Caliber LIMS",
    ...
    compliant({
      ...
      statusText: "Complies",
      checkDescription:
        "QRA read the ammonia result. Like nitrite, it points at biological activity upstream in the system rather than at the sample itself.",
      expected: "Complies — STP-MB-WC-005",
      actual: "Complies",
      expectedSource: "STP-MB-WC-005",
      source: "Caliber LIMS",
    ...
    compliant({
      ...
      statusText: "Complies",
      checkDescription:
        "QRA read the non-volatile residue. What is left after evaporation is what the loop has been leaching.",
      expected: "Complies — STP-MB-WC-005",
      actual: "Complies",
      expectedSource: "STP-MB-WC-005",
      source: "Caliber LIMS",
    ...
    compliant({
      ...
      statusText: "Complies",
      checkDescription:
        "QRA read the oxidisable substances result. It is a second read on organic load, independent of the TOC analyser.",
      expected: "Complies — STP-MB-WC-005",
      actual: "Complies",
      expectedSource: "STP-MB-WC-005",
      source: "Caliber LIMS",
    ...
    compliant({
      ...
      statusText: "Complies",
      checkDescription:
        "QRA read the dissolved solids result against the conductivity above it — the two answer the same question by different routes and should agree.",
      expected: "Complies — STP-MB-WC-005",
      actual: "Complies",
      expectedSource: "STP-MB-WC-005",
      source: "Caliber LIMS",
    ...
    compliant({
      ...
      statusText: "Complies",
      checkDescription:
        "QRA read the hardness result. Calcium and magnesium breaking through say the softening stage of the plant is not holding.",
      expected: "Complies — STP-MB-WC-005",
      actual: "Complies",
      expectedSource: "STP-MB-WC-005",
      source: "Caliber LIMS",
  ...
  logBookSection("waterchem", 5, "Not applicable — chemical testing", "—", "TOC analyser TOC-MB-001"),
  ...
  section("methodval", "Controls", 2, [
    compliant({
      ...
      checkDescription:
        "QRA read the neutraliser efficacy result. It is what shows the neutraliser actually stops the product inhibiting growth, rather than being assumed to.",
    ...
    compliant({
      ...
      checkDescription:
        "QRA read the toxicity result. A neutraliser that suppresses the organism itself would under-count exactly the way the product would, and look like a clean sample.",
    ...
    compliant({
      ...
      checkDescription:
        "QRA read the inoculum control. It fixes how many organisms were actually added, which is the denominator every recovery percentage is calculated from.",
  ...
  section(
    ...
    ["1:10", "1:50", "1:100", "1:500"].map((ratio) =>
      compliant({
        ...
        reference: ratio,
        statusText: "Verified",
        checkDescription: `QRA read the ${ratio} dilution against the prepared volumes. Each ratio is verified on its own because an error in one moves only the results calculated from it, and would pass unnoticed inside the set.`,
  ...
  section("methodval", "Recovery", 4, [
    compliant({
      ...
      statusText: "Within range",
      checkDescription:
        "QRA read the TAMC recovery against the acceptance range. Recovery is what says the method can find organisms in this product — a low figure means routine counts have been under-reading all along.",
    ...
    compliant({
      ...
      statusText: "Within range",
      checkDescription:
        "QRA read the TYMC recovery against the acceptance range. Mould recovery is checked separately because it is the harder of the two to demonstrate.",
      expected: "50 % to 200 % recovery — RULE-MIC-05",
      actual: "78 %",
      expectedSource: sopFor("6"),
      source: "Caliber LIMS",
  ...
  section("methodval", "Method Validation Results", 5, [
    compliant({
      ...
      statusText: "Suitable",
      checkDescription:
        "QRA read the TAMC validation outcome. This is the conclusion the routine MLT test depends on: without it, a passing count means nothing.",
    ...
    compliant({
      ...
      statusText: "Suitable",
      checkDescription:
        "QRA read the TYMC validation outcome against the protocol.",
    ...
    compliant({
      ...
      statusText: "Suitable",
      checkDescription:
        "QRA read the pathogen recovery outcome. Presence testing has to be shown to work in this product before an 'absent' result can be relied on.",
  ...
  logBookSection("methodval", 7, "Soybean casein digest agar", "SCDA-2026-0812", "Incubator INC-MB-004"),
];
